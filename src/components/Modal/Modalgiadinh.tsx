...
const ModalQuanlive: React.FC<ModalProps> = ({ 
  ...
  const handleFamilySave = async () => {
    try {
      if (ngaysudung) {
        const docRef = doc(collection(api, "ticket"), idNgaysudung);
        await updateDoc(docRef, { ngaysudung: ngaysudung.format("DD/MM/YYYY") });
      }
      onClose();
    } catch (err) {
      console.log("Lỗi đổi ngày sử dụng (gói gia đình)", err);
    }
  };
  const handleFamilyDateChange = (date: dayjs.Dayjs | null) => {
    setNgaysudung(date);
  };
    return ( <>
    <Modal
     ...
     <Space wrap style={{ marginTop: '22px', 
      ...
      <Button danger onClick={handleFamilySave} style={{ width: '160px', 
        height: '40px', 
        fontFamily: 'Montserrat', 
        fontSize: '18px', 
        fontWeight: '700', 
